...
# testDirectory        - The directory where the tes bed will be created
...
log = logging.getLogger( 'ATE.FileCountAndSizeBoundaryCheck' )
...
# Class constructor (a Borg)
    def __init__( self ):
        ...
        if( TestOperations.__instance == None ):
            ...
            self.log = logging.getLogger( 'ATE.FileCountAndSizeBoundaryCheck' )
            self.commandConverter = CommandConverter()
            self.log.debug( "FileCountAndSizeBoundaryCheck: TestOperations.__init__() method called" )
            self.__testBedPath = ""
...
# Check the parameters passed
    def __checkParameters( self ):
        self.log.debug( "FileCountAndSizeBoundaryCheck: TestOperations.__checkParameters() method called" )
        ...
        if( self.configMap.__contains__( "maxFileCount" ) != True ):
            self.__checkStatus( False, "Test failed - maxFileCount parameter invalid.")    
...
# Upload / Download the files to and from the slice server
    def __uploadDownload( self, sourcePath, destinationPath ):          
        self.log.debug( "FileCountAndSizeBoundaryCheck: TestOperations.__uploadDownload() method called" )
...
# Compare two directories
    def __compare( self, sourcePath, destinationPath ):
        self.log.debug( "FileCountAndSizeBoundaryCheck: TestOperations.__compare() method called" )
...
# Start executing the test 
    def StartTest( self ):
        self.log.debug( "FileCountAndSizeBoundaryCheck: TestOperations.__StartTest() method called" )
        ...
        if( self.configMap.__contains__( "minFileSize" ) ):
            curFileSize = string.atoi( self.configMap[ "minFileSize" ] )
        
        if( self.configMap.__contains__( "minFileCount" ) ):
            curFileCount = string.atoi( self.configMap[ "minFileCount" ] )

        if( self.configMap.__contains__( "fileSizeIterator" ) ):
            fileSizeIterator = string.atoi( self.configMap[ "fileSizeIterator" ] )
            
        if( self.configMap.__contains__( "fileCountIterator" ) ):
            fileCountIterator = string.atoi( self.configMap[ "fileCountIterator" ] )
            
        while( curFileCount <= string.atoi( self.configMap[ "maxFileCount" ] ) or  curFileSize <= string.atoi( self.configMap[ "maxFileSize" ] ) ):
        # Iterate through all the phases until maxFileCount and maxFileSize has been reached       
            ...
            status = self.__generateTestBed( `curFileCount`, `curFileSize`, self.configMap[ "fileType" ], self.configMap[ "testDirectory" ] )
            ...
            self.log.info( "FileCountAndSizeBoundaryCheck: Successfully completed iteration " + `iteration` + " with a max file size of " + `curFileSize` + " bytes and a file count of " + `curFileCount` )
            
            if( curFileCount <= string.atoi( self.configMap[ "maxFileCount" ] ) ):
                curFileCount += fileCountIterator
            
            if( curFileSize <= string.atoi( self.configMap[ "maxFileSize" ] ) ):
...
def Info():
    log.debug( "FileCountAndSizeBoundaryCheck: Info() method called" )
    testInfo = "This test performs a UDC from the local drive to iSCSI device. The file count and file size is varied until the system crashes or until the celing is reached."
    return testInfo


def Main():
    log.debug( "FileCountAndSizeBoundaryCheck: Main() method called" )          
    testObj = TestOperations()
    if( testObj.StartTest() ):
        log.info( "FileCountAndSizeBoundaryCheck: completed successfully" )
        return( True )
    else:
        return( False )
